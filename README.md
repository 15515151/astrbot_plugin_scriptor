...
TODO 系统已全面升级，支持时间属性、周期性任务、优先级和自动提醒联动。

**核心能力**：
- **时间属性**：支持截止时间（`due_date`）和提前提醒（`advance_minutes`）
- **周期性任务**：支持 Cron 表达式（如 `daily`、`weekly`、`0 9 * * *`）
- **优先级管理**：高/中/低三级优先级，逾期任务自动标记
- **提醒联动**：创建 TODO 时可一键生成关联提醒，完成/删除时自动清理

**生命周期联动**：
```
创建 TODO + need_reminder=True → 自动创建定时任务
完成/删除 TODO → 自动删除关联的定时任务（避免"幽灵提醒"）
周期任务完成 → 自动推延到下一周期（不归档）
```

**Heartbeat 逾期询问**：
当 Heartbeat 触发时，系统会自动检查：
- ⚠️ **逾期任务**：提醒用户关注未完成的事项
- 🔥 **今日高优先级任务**：主动提醒重要任务

**使用示例**：
```
用户: "每天晚上11点提醒我睡觉，提前10分钟"
AI: 调用 add_todo(content="睡觉", due_date="23:00", cron_expression="daily", 
     advance_minutes=10, need_reminder=True)
```
